...
struct backupEndereco {
	...
	int   finalizarPrograma, escolha, i, usuarioEdt, dadoVacinaEdt, usuarioDeletado, dadoEnderecoEdt;
    char *backupId[1000], *id[1000], *nome[1000], *backupNome[1000], *email[1000], *backupEmail[1000], *sexo[1000], *backupSexo[1000], *endereco[1000], *backupEndereco[1000], dadoUsuarioEdt[10], dadoEditado[30], buscarEmail[30];
...
void incluirUsuario(){ // 1
            ...
            *id[i] = rand()%1000;            // ID 
            ...
            enderecos[i] = (struct endereco*)malloc(sizeof(struct endereco));		//ENDERECO
